...
export default function App() {
  ...
  async function fetchPayments() {
    const { data, error } = await supabase.from("month_payments").select("*");
    if (error) {
  ...
  async function togglePaid() {
    ...
    const { error } = await supabase.from("month_payments").upsert({
      month,
      paid: nowPaid,
  ...
  async function clockOff() {
    ...
    const { error } = await supabase
      .from("time_entries")
      .update({ end_time: hmStr(new Date()) })
      .eq("id", openEntry.id);
    if (error) setErr(error.message);
    else fetchEntries();
  }
  ...
  function editEntry(e) {
    setEditId(e.id);
    setMDate(e.work_date);
    setMStart(hm(e.start_time));
    setMEnd(hm(e.end_time));
    setMNote(e.note || "");
  ...
  function exportMonths(keys, filename) {
    ...
    keys.forEach((mk) => {
      ...
      XLSX.utils.book_append_sheet(
        wb,
        buildSheet(mr, monthLabel(mk), payments[mk]),
        safe,
      );
    });
    ...
    <div className="wrap">
      ...
      <div className="card">
        <div className="clock">
          ...
          <div className="btns">
            <button className="in" onClick={clockIn} disabled={!!openEntry}>
              Clock In
            </button>
            <button className="out" onClick={clockOff} disabled={!openEntry}>
              Clock Off
            </button>
          </div>
        </div>
      ...
      <div className="card">
        <div className="toolbar">
          <strong>
            {editId ? `Editing entry — ${mDate}` : "Add entry manually"}
          </strong>
        </div>
        <div className="row">
          <div className="field">
          ...
          <button className="accent" onClick={saveEntry}>
        ...
        <div className="toolbar">
          ...
          <div className="btns">
            ...
            <button
              ...
              onClick={() => exportMonths([month], `Work-Hours-${month}.xlsx`)}
            >
              Export this month
            </button>
            <button
              ...
              onClick={() =>
                exportMonths(
                  ...
                  "Work-Hours-All-Months.xlsx",
                )
              }
            >
              Export all months
            </button>
            <button className="ghost mini" onClick={clearAll}>
              Clear all
            </button>
          </div>
        </div>
              ...
              {rows.map((e) => (
                <tr key={e.id}>
                  ...
                  <td style={{ whiteSpace: "nowrap" }}>
                    <button className="link" onClick={() => editEntry(e)}>
                      Edit
                    </button>
                    <button
        ...
        {rows.length > 0 && (
          <div className="totalbar">
            ...
            <div className="stat">
              ...
              {isPaid && paidInfo?.paid_at && (
                <div className="k" style={{ marginTop: 4 }}>
                  on {paidInfo.paid_at.slice(0, 10)}
                </div>
              )}
            </div>
          </div>
